<x-layouts::app :title="__('Transactions')">
  <main class="flex-1 p-3 sm:p-6 md:p-8 space-y-6 overflow-y-auto relative">
      ...
      <!-- Header Section -->
      <div id="header-top" class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
          ...
          <div class="flex items-center gap-2.5">
              <div class="relative w-full sm:w-auto" x-data="{ open: false }">
                  ...
                  <div x-show="open" x-cloak class="absolute left-0 sm:left-auto sm:right-0 mt-2 w-full sm:w-72 bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl shadow-2xl z-50 p-4 text-xs">
                      <form method="GET" action="{{ route('transaction') }}" @submit.prevent="$el.submit()" class="space-y-3">
                          <div>
                          ...
                          <div class="flex items-center justify-between pt-2 border-t border-[#2a2a2a]">
                              <a href="{{ route('transaction') }}" class="text-zinc-400 hover:text-white transition-colors">Reset</a>
              ...
              <table class="w-full text-left text-xs whitespace-nowrap">
                  ...
                  <tbody class="divide-y divide-[#2a2a2a]">
                      ...
                      <tr class="hover:bg-[#222222] transition-colors">
                          ...
                          <td class="py-3.5 sm:py-4 px-3 sm:px-4 text-zinc-300 text-[13px] local-time" data-utc="{{$transaction->created_at}}">{{$transaction->created_at}}</td>
                          <td class="py-3.5 sm:py-4 px-3 sm:px-4 font-semibold text-[#00e676] text-[13px]">{{ $transaction->Total_Amount }} Ks</td>
                          <td class="py-3.5 sm:py-4 px-3 sm:px-4">
                              <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[13px] font-medium bg-[#00e676]/10 text-[#00e676] border border-[#00e676]/20">
                                  Completed
                              </span>
                          </td>
                      </tr>
      ...
      <!-- Pure HTML Anchor Floating Button (Mobile Only) -->
      <a href="#header-top" class="fixed bottom-6 right-6 z-50 p-3 bg-[#00e676] text-zinc-900 rounded-full shadow-lg active:bg-[#00c853]">
